...
### Q: 如何配置频道（钉钉/飞书）？
**A:** 进入 Channels 页面 → 点击 + Add Channel → 选择平台 → 点击 📖 设置指南按钮查看详细步骤 → 填写凭据 → 选择 Worker → 保存并启用。

---

## 频道（Channels）

频道将即时通讯平台（钉钉、飞书等）连接到 Worker，让你直接在聊天工具中与 AI 对话。

### 钉钉

1. 打开 [钉钉开发者后台](https://open-dev.dingtalk.com/)，创建企业内部应用
2. 在「应用能力」中添加「机器人」，消息接收模式选 **Stream 模式**
3. 创建版本并发布
4. 在「凭证与基础信息」获取 **Client ID** 和 **Client Secret**
5. （可选）在「安全设置 → 服务器出口 IP」添加公网 IP（下载图片/文件时需要）
6. 在 NoWork Channels 页面填入凭据，选择 Worker，保存并启用

> 💡 在钉钉搜索机器人名称即可开始对话，也可以通过「群设置 → 机器人」添加到群聊。
...
4. 启用「机器人」能力
5. 在「权限管理」中批量导入权限 JSON（详见 UI 中的 📖 设置指南）
6. 在「事件与回调」选择 **长连接（WebSocket）** 模式
7. 添加事件：搜索「接收消息」，订阅「接收消息 v2.0」
8. 创建版本并发布

> ⚠️ 飞书操作顺序很关键：必须先启动 NoWork 服务（填写了 App ID/Secret），再在开放平台配置长连接，否则验证会失败。
